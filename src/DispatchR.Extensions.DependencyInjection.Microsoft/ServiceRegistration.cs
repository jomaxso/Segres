...
public static class ServiceRegistration
{
    ...
    {
        var assemblies = markers.Select(x => x.Assembly).Distinct().ToArray();
        ...
        services.TryAddHandlers(typeof(ICommandHandler<,>), assemblies, registrationOption.CommandHandlerLifetime);
        services.TryAddHandlers(typeof(IEventHandler<>), assemblies, registrationOption.MessageHandlerLifetime);
        services.TryAddHandlers(typeof(IStreamHandler<,>), assemblies, registrationOption.StreamHandlerLifetime);


        // MEDIATORS
        services.TryAddSingleton<IDispatcher>(provider => new Dispatcher(provider, assemblies));
